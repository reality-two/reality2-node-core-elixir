defmodule AiReality2Vars.Data do
  ...
  # -----------------------------------------------------------------------------------------------------------------------------------------
  @doc false
  def handle_call(%{command: "get", parameters: %{key: key}}, _from, state),
    do: {:reply, {:ok, %{key => R2Map.get(state, key, nil)}}, state}
  ...
  @doc false
  def handle_cast(%{command: "set", parameters: %{key: key, value: value}}, state),
    do: {:noreply, R2Map.put(state, key, value)}

  def handle_cast(%{command: "set", parameters: %{"key" => key, "value" => value}}, state),
    do: {:noreply, R2Map.put(state, key, value)}

  def handle_cast(%{command: "delete", parameters: %{key: key}}, state),
    do: {:noreply, R2Map.delete(state, key)}
