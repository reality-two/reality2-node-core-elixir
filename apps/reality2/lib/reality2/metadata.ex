defmodule Reality2.Metadata do
  ...
  def set(name, key, value) when is_binary(name),
    do:
      check_existance(String.to_atom(name), fn ->
        GenServer.call(String.to_atom(name), {:set, key, value})
      end)

  def set(name, key, value) when is_atom(name),
    do: check_existance(name, fn -> GenServer.call(name, {:set, key, value}) end)
  ...
  def get(name, key) when is_binary(name),
    do:
      check_existance(String.to_atom(name), fn ->
        GenServer.call(String.to_atom(name), {:get, key})
      end)

  def get(name, key) when is_atom(name),
    do: check_existance(name, fn -> GenServer.call(name, {:get, key}) end)
  ...
  def delete(name, key) when is_binary(name),
    do:
      check_existance(String.to_atom(name), fn ->
        GenServer.call(String.to_atom(name), {:delete, key})
      end)

  def delete(name, key) when is_atom(name),
    do: check_existance(name, fn -> GenServer.call(name, {:delete, key}) end)
  ...
  # -------------------------------------------------------------------------------------------------------------------------------------------
  def all(name) when is_binary(name),
    do:
      check_existance(
        String.to_atom(name),
        fn -> GenServer.call(String.to_atom(name), {:all}) end,
        []
      )

  def all(name) when is_atom(name),
    do: check_existance(name, fn -> GenServer.call(name, {:all}) end, [])
  ...
  def handle_call({:set, key, value}, _from, state),
    do: {:reply, :ok, R2Map.put(state, key, value)}
