defmodule Reality2.Sentant.Comms do
  ...
  defp convert_map_keys(data) when is_list(data),
    do: Enum.map(data, fn x -> convert_map_keys(x) end)
  ...
  defp convert_for_output(data) when is_list(data),
    do: Enum.map(data, fn x -> convert_for_output(x) end)

  defp convert_for_output(data), do: data

  defp find_events_in_automations(automations) do
    Enum.map(automations, fn automation ->
      automation |> R2Map.get(:transitions, []) |> find_events
    end)
    |> List.flatten()
    |> Enum.uniq()
  end
  ...
  defp find_events([transition | rest]) do
    ...
    case R2Map.get(transition, :public, false) do
      true ->
        [
          %{
            event: R2Map.get(transition, :event),
            parameters: R2Map.get(transition, :parameters, %{})
          }
          | find_events(rest)
        ]
  ...
  defp find_signals_in_automations(automations) do
    Enum.map(automations, fn automation ->
      automation |> R2Map.get(:transitions, []) |> find_signals
    end)
    |> List.flatten()
    |> Enum.uniq()
  end
  ...
  defp find_signals([transition | rest]) do
    case R2Map.get(transition, :actions, []) do
      ...
      actions ->
        signals =
          Enum.map(actions, fn action -> get_signal_from_action(action) end) |> List.flatten()
