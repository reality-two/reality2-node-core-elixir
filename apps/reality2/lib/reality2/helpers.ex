defmodule Reality2.Helpers do
  ...
  defmodule R2Process do
    ...
    def whereis(name, module) do
      case GenServer.call(module, {:get, name}) do
        ...
        pid when is_pid(pid) ->
          # Process is no longer running, so deregister
    ...
    # -----------------------------------------------------------------------------------------------------------------------------------------
    def register({:ok, pid}, name, module) when is_atom(name),
      do: register(Atom.to_string(name), pid, module)
    ...
    def register({:ok, pid}, name) when is_atom(name),
      do: register(Atom.to_string(name), pid, __MODULE__)
    ...
    def handle_call({:set, key, value}, _from, state),
      do: {:reply, :ok, Elixir.Map.put(state, key, value)}

    def handle_call({:delete, key}, _from, state),
      do: {:reply, :ok, Elixir.Map.delete(state, key)}

    def handle_call({:get, key}, _from, state),
      do: {:reply, Elixir.Map.get(state, key, nil), state}
    ...
    def get(map, key, default) when is_binary(key),
      do: Elixir.Map.get(map, key, nil) || Elixir.Map.get(map, String.to_atom(key), default)

    def get(map, key, default) when is_atom(key),
      do: Elixir.Map.get(map, key, nil) || Elixir.Map.get(map, Atom.to_string(key), default)
  ...
  defmodule R2Map do
    ...
    def delete(map, key) when is_binary(key),
      do: Elixir.Map.delete(map, key) |> Elixir.Map.delete(String.to_atom(key))

    def delete(map, key) when is_atom(key),
      do: Elixir.Map.delete(map, key) |> Elixir.Map.delete(Atom.to_string(key))
    ...
    def put(map, key, value) when is_atom(key),
      do: Elixir.Map.put(map, Atom.to_string(key), value)
  ...
  defmodule Convert do
    ...
    # Convert to float
    def to_float(value) when is_number(value), do: value + 0.0
