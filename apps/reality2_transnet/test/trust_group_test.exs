defmodule Reality2Transnet.TrustGroupTest do
  ...
  defp generate_identity(name \\ "TestHive") do
    {:ok, identity} = TrustGroup.generate(name)
    identity
  end
  ...
  describe "generate/1" do
    test "returns {:ok, %TrustGroup{}} with expected fields" do
      {:ok, identity} = TrustGroup.generate("MyHive")

      assert %TrustGroup{} = identity
    ...
    test "trust_group_id is derived deterministically from public key" do
      identity = generate_identity()
      assert identity.trust_group_id == TrustGroup.derive_trust_group_id(identity.public_key)
    end

    test "two generates produce different identities" do
      {:ok, id1} = TrustGroup.generate("Hive1")
      {:ok, id2} = TrustGroup.generate("Hive2")

      refute id1.trust_group_id == id2.trust_group_id
  ...
  # ---------------------------------------------------------------------------
  # derive_trust_group_id/1
  # ---------------------------------------------------------------------------

  describe "derive_trust_group_id/1" do
    test "returns a valid UUID v5 format string" do
      identity = generate_identity()
      trust_group_id = TrustGroup.derive_trust_group_id(identity.public_key)
    ...
    test "is deterministic - same public key always produces same UUID" do
      ...
      id1 = TrustGroup.derive_trust_group_id(identity.public_key)
      id2 = TrustGroup.derive_trust_group_id(identity.public_key)
    ...
    test "different public keys produce different UUIDs" do
      ...
      refute TrustGroup.derive_trust_group_id(id1.public_key) ==
               TrustGroup.derive_trust_group_id(id2.public_key)
    end
    ...
    test "version nibble is 5" do
      ...
      parts = String.split(trust_group_id, "-")
  ...
  describe "compressed_id/1" do
    test "returns a 4-byte binary" do
      identity = generate_identity()
      compressed = TrustGroup.compressed_id(identity.trust_group_id)
    ...
    test "is deterministic - same UUID always produces same compressed ID" do
      ...
      c1 = TrustGroup.compressed_id(identity.trust_group_id)
      c2 = TrustGroup.compressed_id(identity.trust_group_id)
    ...
    test "different UUIDs produce different compressed IDs (with high probability)" do
      ...
      refute TrustGroup.compressed_id(id1.trust_group_id) ==
               TrustGroup.compressed_id(id2.trust_group_id)
    end
    ...
    test "compressed ID is first 4 bytes of SHA256 of the UUID string" do
      ...
      assert TrustGroup.compressed_id(uuid) == expected
    end
  end
  ...
  describe "compressed_id_to_hex/1" do
    test "formats a 4-byte binary as 0x prefixed uppercase hex" do
      ...
      assert TrustGroup.compressed_id_to_hex(input) == "0xA3F7B2C1"
    end

    test "pads with leading zeros when value is small" do
      input = <<0x00, 0x00, 0x00, 0x01>>
      assert TrustGroup.compressed_id_to_hex(input) == "0x00000001"
    end

    test "handles all zeros" do
      input = <<0, 0, 0, 0>>
      assert TrustGroup.compressed_id_to_hex(input) == "0x00000000"
    end

    test "handles max value" do
      input = <<0xFF, 0xFF, 0xFF, 0xFF>>
      assert TrustGroup.compressed_id_to_hex(input) == "0xFFFFFFFF"
    end

    test "returns default for non-4-byte input" do
      assert TrustGroup.compressed_id_to_hex(<<1, 2, 3>>) == "0x00000000"
      assert TrustGroup.compressed_id_to_hex(<<1, 2, 3, 4, 5>>) == "0x00000000"
      assert TrustGroup.compressed_id_to_hex("invalid") == "0x00000000"
    end

    test "roundtrips with compressed_id/1" do
      identity = generate_identity()
      compressed = TrustGroup.compressed_id(identity.trust_group_id)
      hex = TrustGroup.compressed_id_to_hex(compressed)
  ...
  describe "sign/2 and verify/3" do
    test "sign returns a 64-byte binary signature" do
      identity = generate_identity()
      signature = TrustGroup.sign(identity, "hello world")
    ...
    test "roundtrip: sign then verify succeeds" do
      ...
      signature = TrustGroup.sign(identity, message)

      assert TrustGroup.verify(identity.public_key, message, signature) == true
    end
    ...
    test "verify fails with wrong public key" do
      ...
      assert TrustGroup.verify(other.public_key, message, signature) == false
    end

    test "verify fails with tampered message" do
      identity = generate_identity()
      signature = TrustGroup.sign(identity, "original")

      assert TrustGroup.verify(identity.public_key, "tampered", signature) == false
    end
    ...
    test "verify fails with tampered signature" do
      ...
      assert TrustGroup.verify(identity.public_key, message, tampered_sig) == false
    end
    ...
    test "verify returns false for mismatched random inputs instead of crashing" do
      ...
      assert TrustGroup.verify(random_key, "msg", random_sig) == false
    end

    test "sign works with binary data (not just strings)" do
      identity = generate_identity()
      binary_data = :crypto.strong_rand_bytes(256)
      signature = TrustGroup.sign(identity, binary_data)

      assert byte_size(signature) == 64
      assert TrustGroup.verify(identity.public_key, binary_data, signature) == true
    end

    test "sign works with empty message" do
      identity = generate_identity()
      signature = TrustGroup.sign(identity, "")

      assert byte_size(signature) == 64
      assert TrustGroup.verify(identity.public_key, "", signature) == true
    end
  end
  ...
  describe "issue_node_cert/4" do
    test "returns {:ok, cert} with expected fields" do
      ...
      {:ok, cert} = TrustGroup.issue_node_cert(identity, "laptop", node_pub)

      assert cert.type == :node_cert
      assert cert.version == 1
      assert cert.trust_group_id == identity.trust_group_id
    ...
    test "respects custom permissions option" do
      ...
      {:ok, cert} =
        TrustGroup.issue_node_cert(identity, "sensor", node_pub,
    ...
    test "respects :never validity" do
      ...
      {:ok, cert} =
        TrustGroup.issue_node_cert(identity, "node", node_pub, validity: :never)
    ...
    test "expires_at is in the future by default" do
      ...
      {:ok, cert} = TrustGroup.issue_node_cert(identity, "node", node_pub)
  ...
  describe "verify_node_cert/2" do
    test "verifies a valid certificate successfully" do
      ...
      {:ok, cert} = TrustGroup.issue_node_cert(identity, "laptop", node_pub)
      {:ok, cert_data} = TrustGroup.verify_node_cert(cert, identity.public_key)

      assert cert_data.trust_group_id == identity.trust_group_id
    ...
    test "rejects certificate signed by different key" do
      ...
      assert {:error, :invalid_signature} =
               TrustGroup.verify_node_cert(cert, other.public_key)
    end
    ...
    test "rejects tampered certificate (changed node_name)" do
      ...
      assert {:error, :invalid_signature} =
               TrustGroup.verify_node_cert(tampered, identity.public_key)
    end
    ...
    test "rejects tampered certificate (changed permissions)" do
      ...
      {:ok, cert} =
        TrustGroup.issue_node_cert(identity, "node", node_pub, permissions: [:read])
    ...
    test "rejects expired certificate" do
      ...
      {:ok, cert} =
        TrustGroup.issue_node_cert(identity, "node", node_pub, validity: 1)
      ...
      assert {:error, :expired} =
               TrustGroup.verify_node_cert(expired_cert, identity.public_key)
    end
    ...
    test "accepts certificate with :never expiry" do
      ...
      assert {:ok, _cert_data} =
               TrustGroup.verify_node_cert(cert, identity.public_key)
    end
    ...
    test "rejects certificate with no signature" do
      ...
      identity = generate_identity()
      assert {:error, :no_signature} = TrustGroup.verify_node_cert(cert, identity.public_key)
    end
  end
  ...
  describe "export_public/1" do
    test "returns a map with expected keys and no private key" do
      ...
      exported = TrustGroup.export_public(identity)

      assert exported.trust_group_id == identity.trust_group_id
  ...
  describe "from_public/1" do
    test "roundtrips through export_public successfully" do
      ...
      {:ok, reconstructed} = TrustGroup.from_public(exported)

      assert reconstructed.trust_group_id == identity.trust_group_id
    ...
    test "rejects mismatched trust_group_id and public_key" do
      identity = generate_identity()
      exported = TrustGroup.export_public(identity)

      # Tamper with trust_group_id
      tampered = %{exported | trust_group_id: "00000000-0000-5000-8000-000000000000"}

      assert {:error, :invalid_identity} = TrustGroup.from_public(tampered)
    end

    test "rejects invalid Base64 public key" do
      data = %{trust_group_id: "something", public_key: "not-valid-base64!!!"}

      assert {:error, :invalid_identity} = TrustGroup.from_public(data)
    end

    test "uses default name when name is missing" do
      identity = generate_identity()
      exported = TrustGroup.export_public(identity) |> Map.delete(:name)
    ...
    test "can verify signatures using reconstructed public identity" do
      ...
      assert TrustGroup.verify(reconstructed.public_key, message, signature) == true
    end
  end
  ...
  describe "full certification roundtrip" do
    test "cert issued by identity can be verified using exported public info" do
      ...
      # Issue a certificate
      {:ok, cert} = TrustGroup.issue_node_cert(identity, "field-sensor", node_pub)

      # Export public info (as would be shared over network)
      exported = TrustGroup.export_public(identity)

      # Receiving node reconstructs public identity
      {:ok, public_identity} = TrustGroup.from_public(exported)

      # Verify the certificate using only the public key
      {:ok, cert_data} = TrustGroup.verify_node_cert(cert, public_identity.public_key)
    ...
    test "can extract node public key from certificate" do
      ...
      {:ok, cert} = TrustGroup.issue_node_cert(identity, "node1", node_pub)
      {:ok, extracted} = TrustGroup.get_node_public_key(cert)
  ...
  describe "edge cases" do
    test "generate with empty name string" do
      {:ok, identity} = TrustGroup.generate("")
      assert identity.name == ""
      assert is_binary(identity.trust_group_id)
    end

    test "generate with unicode name" do
      {:ok, identity} = TrustGroup.generate("Hive-\u00E9\u00E8\u00EA")
      assert identity.name == "Hive-\u00E9\u00E8\u00EA"
    end

    test "compressed_id of two different UUID strings differs" do
      c1 = TrustGroup.compressed_id("aaaaaaaa-aaaa-5aaa-8aaa-aaaaaaaaaaaa")
      c2 = TrustGroup.compressed_id("bbbbbbbb-bbbb-5bbb-8bbb-bbbbbbbbbbbb")
      refute c1 == c2
    end
    ...
    test "sign and verify with large message" do
      ...
      signature = TrustGroup.sign(identity, large_message)

      assert TrustGroup.verify(identity.public_key, large_message, signature) == true
    end
    ...
    test "multiple certificates can be issued by same identity" do
      ...
      {:ok, cert1} = TrustGroup.issue_node_cert(identity, "node1", pub1)
      {:ok, cert2} = TrustGroup.issue_node_cert(identity, "node2", pub2)

      assert {:ok, _} = TrustGroup.verify_node_cert(cert1, identity.public_key)
      assert {:ok, _} = TrustGroup.verify_node_cert(cert2, identity.public_key)
