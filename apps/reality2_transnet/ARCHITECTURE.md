...
### BLE Layer
- `Reality2Transnet.Bluetooth` - BLE beacon, GATT server, discovery
- `Reality2Transnet.GattProtocol` - GATT protocol for device-to-device
- `Reality2Transnet.Action` - Rust NIFs for BlueZ integration
- `Reality2Transnet.R2Mesh` - Simple BLE relay mesh (works on any BlueZ)
- `Reality2Transnet.BLEMesh` - Full BLE Mesh (requires BlueZ 5.47+, future)

### WiFi Layer
- `Reality2Transnet.Wifi` - WiFi hotspot/client operations via NetworkManager
- `Reality2Transnet.ConnectionManager` - Hotspot creation, client connections
- `Reality2Transnet.ConnectionAssessor` - Connection quality, handover decisions
- `Reality2Web.MeshController` - HTTP endpoints for mesh queries

### LoRa Layer (Optional)
- `Reality2Transnet.LoRaMesh` - LoRa mesh via USB dongle (auto-detects hardware)

### Management Layer
- `Reality2Transnet.PeerManager` - Track discovered peers across all transports
- `Reality2Transnet.Main` - Sentant plugin interface

### Integration Layer
- `Reality2Wfs.Router` - Location-transparent Sentant routing
...
Reality2Transnet.Bluetooth.get_state()
...
{:ok, sentants} = Reality2Transnet.WifiServer.query_peer_sentants(peer_ipv6)

# Send command to remote sentant
{:ok, response} = Reality2Transnet.WifiServer.send_to_peer(
  peer_ipv6,
  "sentant-uuid",
  "read",
  %{unit: "celsius"}
...
# Get specific peer
{:ok, peer} = Reality2Transnet.PeerManager.get_peer("node-uuid")

# Check transport
:wifi_mesh = Reality2Transnet.PeerManager.get_transport("node-uuid")
```
