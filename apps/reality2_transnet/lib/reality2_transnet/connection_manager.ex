defmodule Reality2Transnet.ConnectionManager do
  @moduledoc """
  ...
  **Author**
  ...
  alias Reality2Transnet.{Wifi, PeerManager}
  ...
  defp notify_clients_of_sentant_change(state) do
    ...
    if Code.ensure_loaded?(Reality2Wfs.Router) and function_exported?(Reality2Wfs.Router, :refresh_topology, 0) do
  ...
  defp perform_start_hosting(state) do
    ...
    #   Config: config :reality2_transnet, site_id: "WAIROA"
    #   Default: "NODE"
  ...
  defp handle_wifi_connection_lost(state, reason) do
    ...
    Task.start(fn ->
      ...
      Process.sleep(1000)
      Reality2Transnet.ConnectionAssessor.assess_now()
    end)
  ...
  defp cleanup_host_routes(host_node_id) do
    ...
    Reality2Transnet.PeerManager.update_peer_sentants(host_node_id, [])
      ...
      {:ok, %Finch.Response{status: 200, body: response_body}} ->
        case Jason.decode(response_body) do
          {:ok, %{"data" => %{"sentantAll" => host_sentants} = data}} ->
            ...
            Reality2Transnet.PeerManager.update_peer_sentants(host_node_id, host_sentants)
            ...
            all_peers = Reality2Transnet.PeerManager.get_all_peers()
      ...
      {:ok, %Finch.Response{status: 200, body: response_body}} ->
        case Jason.decode(response_body) do
          {:ok, %{"data" => %{"sentantAll" => sentants}}} ->
            ...
            Reality2Transnet.PeerManager.update_peer_sentants(host_node_id, sentants)
  ...
  defp update_wfs_routing_table(peer_node_id, peer_ip, peer_sentants) do
    ...
    peer_node_name = case Reality2Transnet.PeerManager.get_peer(peer_node_id) do
      {:ok, peer} -> Map.get(peer, :node_name, "Unknown")
      _ -> "Unknown"
    end
