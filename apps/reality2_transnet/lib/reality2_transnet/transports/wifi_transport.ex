defmodule Reality2Transnet.Transports.WiFiTransport do
  @moduledoc """
  ...
  **Author**
  ...
  @impl true
  def available? do
    Code.ensure_loaded?(Reality2Transnet.ConnectionManager) and
      Process.whereis(Reality2Transnet.ConnectionManager) != nil and
  ...
  def handle_incoming(raw_data) do
    case decode_message(raw_data) do
      {:ok, message} ->
        Reality2Transnet.MeshRouter.handle_incoming(message, :wifi_hotspot)
        :ok
  ...
  def get_stats do
    if Code.ensure_loaded?(Reality2Transnet.ConnectionManager) do
      try do
        case GenServer.call(Reality2Transnet.ConnectionManager, :get_health_metrics, @connection_status_timeout) do
          {:ok, metrics} ->
            %{
              available: available?(),
  ...
  def get_peers do
    if Code.ensure_loaded?(Reality2Transnet.ConnectionManager) do
      try do
        case GenServer.call(Reality2Transnet.ConnectionManager, :get_status, @connection_status_timeout) do
          {:ok, status} ->
  ...
  defp get_connection_state do
    ...
    case Process.whereis(Reality2Transnet.ConnectionManager) do
      nil ->
        :disconnected
  ...
  defp safe_get_connection_status do
    try do
      GenServer.call(Reality2Transnet.ConnectionManager, :get_status, @connection_status_timeout)
    catch
