defmodule Reality2Transnet.Transports.BLETransport do
  @moduledoc """
  ...
  **Author**
  ...
  @impl true
  def available? do
    Code.ensure_loaded?(Reality2Transnet.Bluetooth) and
  ...
  def broadcast(message) do
    if available?() do
      ...
      if byte_size(encoded) <= @max_payload_size + 6 do  # 6 bytes header
        Reality2Transnet.Bluetooth.broadcast_mesh_message(encoded)
        :ok
      else
  ...
  def handle_incoming(raw_data) do
    case decode_from_ble(raw_data) do
      {:ok, message} ->
        Reality2Transnet.MeshRouter.handle_incoming(message, :ble)
        :ok
  ...
  def get_stats do
    if available?() do
      state = Reality2Transnet.Bluetooth.get_state()
      %{
        available: true,
