defmodule Reality2Transnet.Transports.LoRaTransport do
  @moduledoc """
  ...
  **Author**
  ...
  @impl true
  def available? do
    Code.ensure_loaded?(Reality2Transnet.LoRaMesh) and
      Reality2Transnet.LoRaMesh.available?()
  end
  ...
  def broadcast(message) do
    if available?() do
      ...
      if payload_size > @max_inner_payload do
        ...
        Reality2Transnet.LoRaMesh.send_raw(encoded)
      end
    else
      {:error, :not_available}
    end
  ...
  def handle_incoming(raw_data) do
    case decode_message(raw_data) do
      {:ok, message} ->
        Reality2Transnet.MeshRouter.handle_incoming(message, :lora)
        :ok
  ...
  def get_stats do
    if available?() do
      Reality2Transnet.LoRaMesh.get_stats()
    else
      %{available: false}
    end
  end
