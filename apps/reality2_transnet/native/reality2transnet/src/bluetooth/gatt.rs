...
/// Characteristic data that supports chunked reads for clients that don't use Read Blob.
/// Each read advances an internal offset; writing resets the offset.
#[derive(Clone)]
struct ChunkedCharacteristicData {
    value: Arc<Mutex<Vec<u8>>>,
...
impl ChunkedCharacteristicData {
    ...
    /// Returns empty when all data has been read.
    fn read_next_chunk(&self) -> Vec<u8> {
        let value = self.value.lock().unwrap();
        let mut offset = self.read_offset.lock().unwrap();

        if *offset >= value.len() {
            return vec![];
        }

        let end = std::cmp::min(*offset + self.chunk_size, value.len());
        let chunk = value[*offset..end].to_vec();
        *offset = end;
        chunk
    }

    /// Read from a specific offset (for clients that support Read Blob).
    fn read_from_offset(&self, offset: usize) -> Vec<u8> {
        let value = self.value.lock().unwrap();
        if offset >= value.len() {
            vec![]
        } else {
    ...
    /// Write new data, resetting the read offset.
    fn write(&self, new_value: Vec<u8>) {
        *self.value.lock().unwrap() = new_value;
        *self.read_offset.lock().unwrap() = 0;
    }

    fn read_full(&self) -> Vec<u8> {
        self.value.lock().unwrap().clone()
    }
...
async fn run_gatt_server(
    ...
    // Use chunked reads for hive join (large encrypted results won't fit in one read)
    let hive_join_data = ChunkedCharacteristicData::new(500);
    ...
    let service = Service {
        ...
        characteristics: vec![
            ...
            Characteristic {
                ...
                read: Some(CharacteristicRead {
                    read: true,
                    fun: Box::new(move |req_data| {
                        let data = hive_join_data_read.clone();
                        Box::pin(async move {
                            let offset = req_data.offset as usize;
                            if offset > 0 {
                                // Client supports Read Blob — use offset
                                let chunk = data.read_from_offset(offset);
                                eprint!("[debug] GATT HIVE_JOIN char read (blob) - offset {} returning {} bytes\r\n", offset, chunk.len());
                                Ok(chunk)
                            } else {
                                // Client sent regular Read Request (offset=0) — return next chunk
                                let chunk = data.read_next_chunk();
                                let total = data.read_full().len();
                                eprint!("[debug] GATT HIVE_JOIN char read (chunked) - returning {} bytes (total: {})\r\n", chunk.len(), total);
                                Ok(chunk)
                            }
                        })
                    }),
