...
pub fn reset_nodes(handle: ResourceArc<WatchHandle>) -> rustler::Atom {
    handle.send_command(ResetCommand::ClearAll);
    atoms::ok()
}

/// Pauses BLE scanning to allow GATT client connections on the same adapter.
/// Discovery stream is dropped, freeing the adapter for client operations.
#[rustler::nif]
pub fn pause_watching(handle: ResourceArc<WatchHandle>) -> rustler::Atom {
    handle.send_command(ResetCommand::Pause);
    atoms::ok()
}

/// Resumes BLE scanning after a pause. Creates a new discovery stream.
#[rustler::nif]
pub fn resume_watching(handle: ResourceArc<WatchHandle>) -> rustler::Atom {
    handle.send_command(ResetCommand::Resume);
    atoms::ok()
}
...
async fn run_continuous_watch(
    ...
    loop {
        // When paused, wait for Resume or Shutdown without running discovery.
        // This frees the adapter for GATT client connections.
        if paused {
            tokio::select! {
            ...
            tokio::select! {
                ...
                Some(cmd) = reset_rx.recv() => {
                    match cmd {
                        ...
                        ResetCommand::Pause => {
                            paused = true;
                            break; // Break inner loop, dropping discover stream (stops scanning)
                        }
                        ResetCommand::Resume => {} // Already running
                    }
                }
