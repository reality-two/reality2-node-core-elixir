...
//! [18-21] Compressed Hive ID (4 bytes) - SHA256(hive_uuid)[0:4], 0x00000000 if no hive
//! [22]    RSSI @ 1m (1 byte) - Calibrated signal strength
//! [23]    Hosting Priority (1 byte) - WiFi hosting priority (0-100)
//! ```
...
/// - `hive_compressed` - 4-byte compressed hive ID (SHA256(hive_uuid)[0:4]), or [0,0,0,0] if not in hive
...
///   <<0xA3, 0xF7, 0xB2, 0xC1>>,  # compressed hive ID (4 bytes)
///   -59,
///   "R2Node_A3F7",
...
pub fn start_broadcast<'a>(
    ...
    company_id: u16,
    uuid_str: String,
    hive_compressed: rustler::Binary<'a>,
    rssi_at_1m: i8,
    node_name: String,
    hosting_priority: u8,
    ...
    // Ensure hive_compressed is exactly 4 bytes
    let hive_bytes = hive_compressed.as_slice();
    let hive_id: [u8; 4] = if hive_bytes.len() >= 4 {
        [hive_bytes[0], hive_bytes[1], hive_bytes[2], hive_bytes[3]]
    } else {
        [0, 0, 0, 0]  // No hive
    };
...
/// - `hive_compressed` - 4-byte compressed hive ID
...
async fn run_beacon_advertisement(
    ...
    // Build 24-byte AltBeacon payload with node identification and hive membership
    let payload = build_altbeacon_payload(uuid, hive_compressed, rssi_at_1m, hosting_priority);
...
/// - Bytes 18-21: Compressed Hive ID (4 bytes) - SHA256(hive_uuid)[0:4], 0 if no hive
...
/// - `hive_compressed` - 4-byte compressed hive ID (0x00000000 if not in a hive)
...
fn build_altbeacon_payload(
    ...
    // Bytes 18-21: Compressed Hive ID (4 bytes)
    payload.extend_from_slice(&hive_compressed);
