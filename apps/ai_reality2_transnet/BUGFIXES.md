...
defp send_via_wifi_mesh(peer, sentant_id, event, params, pass) do
  ...
  # HTTP POST to http://[ipv6]:8080/sentants/:id/send
...
curl http://[fe80::1234:5678:90ab:cdef%mesh0]:8080/sentants

# Send command to remote sentant
curl -X POST http://[fe80::...%mesh0]:8080/sentants/UUID/send \
  -H "Content-Type: application/json" \
  -d '{"event": "read", "parameters": {"unit": "celsius"}}'
```
...
# → HTTP POST to http://[peer-ipv6]:8080/sentants/:id/send
```
