...
1. **GET /sentants** - Query all Sentants
...
2. **POST /sentants/:id/send** - Send event to Sentant
...
3. **GET /info** - Query node info with mesh details
...
5. Node B queries Sentants via WiFi HTTP
   └─> GET http://[fe80::...]:8080/sentants
   └─> Receives full Sentant list (no size limits)

6. Node B sends commands via WiFi HTTP
   └─> POST http://[fe80::...]:8080/sentants/:id/send
```
...
- `AiReality2Transnet.WifiServer` - HTTP server for Sentant queries
- `AiReality2Transnet.WifiServer.Router` - Plug router with endpoints
...
config :ai_reality2_transnet,
  wifi_server_port: 8080  # HTTP server port
```
