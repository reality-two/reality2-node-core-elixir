  ...
  defp phase_1(cfg, verbose?) do
    ...
    [
      ...
      test("HiveDirectory has >= 2 nodes", fn ->
        directory = AiReality2Transnet.HiveDirectory.get_directory()
  ...
  defp phase_2(cfg, verbose?) do
    ...
    [
      ...
      test("MeshRouter relay count > 0", fn ->
        stats = AiReality2Transnet.MeshRouter.get_stats()
      ...
      test("PeerManager stats valid", fn ->
        stats = AiReality2Transnet.PeerManager.get_stats()
  ...
  defp phase_4(_cfg, verbose?) do
    [
      ...
      test("Small event delivered via LoRa", fn ->
        case AiReality2Transnet.LoRaMesh.broadcast_event("hardware_test", "lora_ping", %{t: "test"}) do
          :ok -> :ok
          {:ok, _} -> :ok
