defmodule AiReality2Transnet.WifiServer do
  ...
  ## Port 4005 vs Port 8080

  **IMPORTANT:** Both ports expose **public information only** (privacy-filtered).
  GraphQL is designed to only ever expose the public API surface, never internal state.

  The difference is in **write operations**:

  - **Port 4005 (Main API)**:
    - Can query Sentants (read-only) ✅
    - Can send events to Sentants ✅
    - Can **create/delete Sentants** (if node allows it) ✅
  ...
  2. **Dynamically via Port 4005** - Only if environment variable permits:
     - Node is "open": Sentants can be created/deleted via GraphQL
     - Node is "closed": Sentants are startup-only, no dynamic changes

  Port 8080 never allows Sentant creation/deletion, regardless of node configuration.
  ...
  **GraphQL in Reality2 is designed to ONLY expose public information**, regardless of port.
  ...
  Both port 4005 and port 8080 follow the same privacy model:
  ...
  **Sentants are designed with privacy-first architecture:**
  ...
  1. Starts a Cowboy HTTP server on port 8080 (configurable)
  2. Listens on all interfaces (0.0.0.0) to accept mesh connections
  3. Routes `/graphql` to Plug.Router
  4. Executes queries via `Absinthe.run(query, Reality2Web.Schema)`
  5. Post-filters responses to remove private data
  6. Tracks request statistics
  ...
  **Note:** Response does NOT include:
  ...
  {:ok, pid} = AiReality2Transnet.WifiServer.start_link([])

  # Check status
  AiReality2Transnet.WifiServer.get_status()
  # => %{port: 8080, requests_total: 42, requests_successful: 40}
  ```
  ...
  ```bash
  # Query sentants
  curl -X POST "http://[fe80::peer-ipv6%mesh0]:8080/graphql" \
  ...
  **WiFi Mesh = Semi-Trusted Environment:**
  ...
  Server port is configurable via module attribute (default: 8080).
    ...
    case AiReality2Transnet.Wifi.check_dependencies() do
      ...
      {:ok, :all_available} ->
        ...
        case AiReality2Transnet.Wifi.list_adapters() do
          {:ok, [_ | _] = adapters} ->
            # Start HTTP server
            port = Application.get_env(:ai_reality2_transnet, :wifi_server_port, @default_port)

            # Use Plug.Cowboy to start HTTP server
            {:ok, _pid} = Plug.Cowboy.http(__MODULE__.Router, [], port: port)
            ...
            Logger.info("[WifiServer] HTTP server started on port #{port}")
            Logger.info("[WifiServer] Found #{length(adapters)} WiFi adapter(s)")
            {:ok, state}

          {:ok, []} ->
            Logger.warning("[WifiServer] No WiFi adapters available - server not started")
