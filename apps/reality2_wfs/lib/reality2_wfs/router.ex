defmodule Reality2Wfs.Router do
  @moduledoc """
  ...
  Programmatic usage:
      ...
      Reality2Wfs.Router.send_to_sentant("Zen Quote", "init", %{})

      # Send to all nodes with "Sensor"
      Reality2Wfs.Router.send_to_sentant("*|Sensor", "ping", %{})
      # => {:ok, %{local: 1, remote: 3, results: [...]}}

      # Target specific node
      Reality2Wfs.Router.send_to_sentant("R2Node_A3F7|Zen Quote", "init", %{})

      # Locate a sentant locally
      Reality2Wfs.Router.locate("Zen Quote")
      # => {:ok, :local, "uuid..."}

      # Locate across all nodes
      Reality2Wfs.Router.locate("*|Sensor")
  ...
  **Author**
  ...
  defp handle_broadcast_all(event, params, passthrough, sender, state) do
    ...
    {remote_count, remote_results} = if Code.ensure_loaded?(Reality2Transnet.PeerManager) do
  ...
  defp peer_exists?(node_id) do
    if Code.ensure_loaded?(Reality2Transnet.PeerManager) do
      case Reality2Transnet.PeerManager.get_peer(node_id) do
        {:ok, _} -> true
        _ -> false
      end
    else
  ...
  defp send_to_remote_with_transport(node_id, sentant_identifier, event, params, passthrough, sender) do
    if Code.ensure_loaded?(Reality2Transnet.PeerManager) do
      case Reality2Transnet.PeerManager.best_transport(node_id) do
        {:ok, :wifi, _info} ->
  ...
  defp send_via_mesh_router(node_id, sentant_identifier, event, params, passthrough, sender) do
    if Code.ensure_loaded?(Reality2Transnet.MeshRouter) do
      ...
      case Reality2Transnet.MeshRouter.send_signal("wfs_router", target, event, full_params) do
  ...
  defp send_to_remote_gatt(node_id, sentant_id, event, parameters, passthrough, sender) do
    ...
    with true <- Code.ensure_loaded?(Reality2Transnet.PeerManager),
         {:ok, peer} <- Reality2Transnet.PeerManager.get_peer(node_id) do
  ...
  defp send_via_graphql_if_available(node_id, sentant_id, event, parameters, passthrough, sender) do
    with true <- Code.ensure_loaded?(Reality2Transnet.PeerManager),
         {:ok, peer} <- Reality2Transnet.PeerManager.get_peer(node_id),
  ...
  defp refresh_topology_cache(state) do
    ...
    {peer_sentants, remote_locations} = if Code.ensure_loaded?(Reality2Transnet.PeerManager) do
      peers = Reality2Transnet.PeerManager.get_all_peers()
