...
# ****************************************************************************************************
defmodule Reality2Web.Schema.Enums do
  @moduledoc false
  # ----------------------------------------------------------------------------------------------------
  # Public Interface
  # ----------------------------------------------------------------------------------------------------
  ...
  def convert({:ok, the_map}, the_key) when is_map(the_map),
    do:
      {:ok,
       the_map
       |> Map.replace(the_key, convert_item(Map.get(the_map, the_key), Map.get(enums(), the_key)))}

  def convert({:ok, data}, _), do: {:ok, data}
  def convert({:error, error}, _), do: {:error, error}

  def convert(the_list, the_key) when is_list(the_list),
    do: convert_list(the_list, Map.get(enums(), the_key), the_key)

  def convert(the_map, the_key) when is_map(the_map),
    do:
      the_map
      |> Map.replace(the_key, convert_item(Map.get(the_map, the_key), Map.get(enums(), the_key)))
  ...
  defp convert_item(the_atom, reference) when is_atom(the_atom),
    do: get_index(reference, the_atom, 0, 0)
