defmodule AiReality2Geospatial.Data do
  ...
  # -----------------------------------------------------------------------------------------------------------------------------------------
  @doc false
  def handle_call(%{command: "get"}, _from, state),
    do:
      {:reply,
       {:ok,
        R2Map.get(state, "location", %{
          latitude: 0.0,
          longitude: 0.0,
          altitude: 0.0,
          geohash: "",
  ...
  def handle_call(%{command: "search", parameters: parameters}, _from, state),
    do: {:reply, {:ok, %{sentants: get_nearby_sentants(state, parameters)}}, state}
      ...
      nil ->
        case is_nil(latitude) or is_nil(longitude) do
          ...
          false ->
            ...
            GenServer.call(AiReality2Geospatial.GeohashSearch, %{
              command: "store",
  ...
  defp get_nearby_sentants(state, parameters) do
    case R2Map.get(state, "location") do
      ...
      location ->
        # Radius in meters
        radius = R2Map.get(parameters, "radius", 10.0)
            ...
            if pid == self() do
              ...
              case GenServer.call(pid, %{command: "get"}) do
                ...
                {:ok, location2} ->
                  ...
                  if (dist <= radius && sentant_radius == 0.0) ||
                       (sentant_radius > 0.0 && dist <= radius && dist <= sentant_radius) do
