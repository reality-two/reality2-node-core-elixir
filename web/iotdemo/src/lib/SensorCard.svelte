...
<script lang="ts">
    ...
    const handleOrientation = (event: any) => {
        if (sensor_active) {
            ...
            if (new_sensor_in !== sensor_in && now - lastExecutionTime >= 200) {
                ...
                r2_node.sentantSend(sentant.id, "set_sensor", {
                    sensor: sensor_in,
                });
    ...
    onMount(() => {
        ...
        if (
            sentant.name == "monitor" ||
            sentant.name == ".deleted" ||
            sentant.name == "view"
        )
            return;
...
{#if sentant.name !== "monitor" && sentant.name !== ".deleted" && sentant.name !== "view"}
    <Card>
        ...
        <Content>
            ...
            {:else}
                <Button
                    ...
                    on:click={(_e) => {
                        sensor_active = true;
                    }}>Press to use sensor</Button
                >
            {/if}
        </Content>
        <Content>
            {#if sensor_active}
                <Text ui large grey>Rotate for colour</Text>
            {:else}
                <Text ui large grey>Choose a colour</Text>
                <Buttons ui fluid>
                    <Button
                        ui
                        red
                        on:click={(_e) => colour_button_pressed(sentant.id, 10)}
                    >
                        <Text ui large>&nbsp;</Text>
                    </Button>
                    <Button
                        ui
                        yellow
                        on:click={(_e) => colour_button_pressed(sentant.id, 80)}
                    >
                        <Text ui large>&nbsp;</Text>
                    </Button>
                    <Button
                        ...
                        green
                        on:click={(_e) =>
                            colour_button_pressed(sentant.id, 150)}
                    >
                        <Text ui large>&nbsp;</Text>
                    </Button>
                    <Button
                        ...
                        blue
                        on:click={(_e) =>
                            colour_button_pressed(sentant.id, 230)}
                    >
                        <Text ui large>&nbsp;</Text>
                    </Button>
                    <Button
                        ...
                        purple
                        on:click={(_e) =>
                            colour_button_pressed(sentant.id, 290)}
                    >
                        <Text ui large>&nbsp;</Text>
                    </Button>
                </Buttons>
        ...
        <Content extra>
            <p>
                <Label ui huge basic _={connected ? "blue" : "grey"} fluid
                    >{sentant.name}</Label
                >
            </p>
...
{:else}
    <Message ui teal large>
        <Header>Device connection invalid</Header>
    </Message>
{/if}
