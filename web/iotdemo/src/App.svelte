...
<script lang="ts">
                    ...
                    transitions: [
                        ...
                        },
                        {
                            ...
                            actions: [
                                ...
                                {
                                    command: "send",
                                    parameters: { event: "update", to: "view" },
                                },
                                {
                                    command: "signal",
                                    public: true,
                                    parameters: { event: "update" },
                                },
                            ],
                        },
                    ],
    ...
    if (id_query == null && name_query == null) {
        setTimeout(() => {
            ...
            r2_node
                .sentantLoad(JSON.stringify(view_sentant))
                .then((data) => {
                    ...
                    r2_node.awaitSignal(id, "update", (signal_data: any) => {
                        if (R2.JSONPath(signal_data, "status") == "connected") {
                            ...
                            let name = R2.JSONPath(
                                signal_data,
                                "parameters.name",
                            );
                            let sensor = R2.JSONPath(
                                signal_data,
                                "parameters.sensor",
                            );
    ...
    function newDevice() {
        ...
        r2_node
            ...
            .then((data) => {
                ...
                var sentantDefinition = JSON.stringify(template).replace(
                    /__name__/gi,
                    newName,
                );
                ...
                r2_node
                    .sentantLoad(sentantDefinition)
                    .then((data) => {
                        // Get the ID of the new Sentant
                        var sentantID = R2.JSONPath(
                            data,
                            "data.sentantLoad.id",
                        );
...
<main>
    <Segment ui bottom attached grey>
        ...
        {:else if state == "connect"}
            <!--------------------------------------------------------------------------------------------->
            <Message ui blue large>
                <Header>Connect your device</Header>
            </Message>
        ...
        {:else if state == "error"}
            ...
            <Message ui negative large>
                <Header>Something bad happened</Header>
            </Message>
            <!--------------------------------------------------------------------------------------------->
        ...
        {:else if none_or_monitor_only(sentantData)}
            ...
            <Message ui teal large>
                <Header>No Devices Connected</Header>
            </Message>
            <!--------------------------------------------------------------------------------------------->
        {:else if state == "id"}
            <!--------------------------------------------------------------------------------------------->
