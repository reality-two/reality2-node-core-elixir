...
const shape = {
  ...
  colour: 150,
  tooltip:
    "An antenna to retrieve information from an internet connected resource that uses a POST API.",
...
function construct(plugin: any) {
  let description = R2.JSONPath(plugin, "description")
    ? R2.JSONPath(plugin, "description")
    : "";
  ...
  if (plugin) {
    ...
    let headers = reality2_plugin_header.construct(
      R2.JSONPath(plugin, "headers"),
    );
    if (headers) block["inputs"]["headers"] = { block: headers };

    // Check if there are query parameters
    let parameters = reality2_plugin_parameter.construct(
      R2.JSONPath(plugin, "parameters"),
    );
