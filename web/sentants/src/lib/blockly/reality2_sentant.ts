...
const shape = {
  ...
  colour: 50,
  tooltip:
    "A Reality2 Sentient Digital Agent, otherwise known as a Bee, or a Sentant.",
...
function process(block: any, generator: any): string | [string, number] | null {
  ...
  if (block.getFieldValue("description") !== "")
  ...
  if (plugins != "") {
    sentant["plugins"] = splitConcatenatedJSON(plugins, false).map(
      (plugin: any) => {
  ...
  if (automations != "") {
    sentant["automations"] = splitConcatenatedJSON(automations, false).map(
      (automation: any) => {
...
function construct(sentant: any) {
  let description = R2.JSONPath(sentant, "description")
    ? R2.JSONPath(sentant, "description")
    : "";
  if (sentant) {
    ...
    if (
      R2.JSONPath(sentant, "keys.encryption_key") ||
      R2.JSONPath(sentant, "keys.decryption_key")
    ) {
      let keys = reality2_encrypt_decrypt_keys.construct(
        R2.JSONPath(sentant, "keys"),
      );
