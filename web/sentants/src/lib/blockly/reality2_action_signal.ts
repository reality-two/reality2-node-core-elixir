...
const shape = {
  ...
  colour: 300,
  tooltip:
    "Send a signal to any connected devices, with some additional information to add to the data flow.",
...
function process(block: any, generator: any): string | [string, number] | null {
  ...
  if (Object.keys(params).length !== 0)
    action["parameters"]["parameters"] = params;
...
function construct(action: any) {
  let access = "private";
  if (
    R2.JSONPath(action, "parameters.public") === true ||
    R2.JSONPath(action, "public") === true
  )
    access = "public";
  ...
  if (action) {
    ...
    let parameters = reality2_action_parameter.construct(
      R2.JSONPath(action, "parameters.parameters"),
    );
