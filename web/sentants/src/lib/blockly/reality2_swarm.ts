...
const shape = {
  ...
  colour: 20,
  tooltip:
    "A swarm of Sentient Digital Agents, otherwise known as Sentants or Bees.",
...
function process(block: any, generator: any): string | [string, number] | null {
  ...
  if (sentants != "") {
    swarm["sentants"] = splitConcatenatedJSON(sentants, false).map(
      (sentant: any) => {
        return sentant["sentant"];
      },
    );
  }
