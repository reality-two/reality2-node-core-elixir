...
// ----------------------------------------------------------------------------------------------------
export function splitConcatenatedJSON(
  jsonString: string,
      ...
      if (plugin_name == "ai.reality2.vars") {
        ...
        switch (command) {
          case "set":
            ...
            if (!latitude && !longitude && !geohash) {
              action_block =
                ai_reality2_geospatial_set_simple.construct(action);
              if (action_block && acc) {
                action_block["next"] = { block: acc };
              }
            } else if (!(latitude || longitude) && geohash) {
              action_block =
                ai_reality2_geospatial_set_geohash.construct(action);
              if (action_block && acc) {
                action_block["next"] = { block: acc };
              }
            } else {
        ...
        if (parameters && Object.keys(parameters).length > 0) {
          ...
          if (R2.JSONPath(action, "command") === "send") {
            action_block =
              reality2_action_send_plugin_no_params_no_event.construct(action);
            if (action_block && acc) {
              action_block["next"] = { block: acc };
            }
          } else {
            action_block =
              reality2_action_send_plugin_no_params.construct(action);
            if (action_block && acc) {
              action_block["next"] = { block: acc };
            }
          }
          ...
          case "set":
            ...
            if (value != null) {
              switch (value) {
                case "radius":
                  action_block =
                    ai_reality2_geospatial_set_radius.construct(action);
                  break;
                case "geohash":
                  action_block =
                    ai_reality2_geospatial_set_geohash.construct(action);
          ...
          case "send":
            ...
            if (delay > 0) {
              ...
              if (parameters && Object.keys(parameters).length > 0) {
                ...
                action_block =
                  reality2_action_send_now_no_params.construct(action);
                if (action_block && acc) {
                  action_block["next"] = { block: acc };
                }
              }
