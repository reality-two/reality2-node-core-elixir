...
@export										var hover_select_color:Color = Color(1,1,1, 0.2)
...
func _draw():
	...
	if selection == -1 and first_in_center:
		draw_circle(offset, arc_inner_radius, segment_color)
		...
		for i in child_count:
			...
			if (arc_inner_radius < radius) and (selection == i):
				...
				draw_polygon(
					...
					PackedColorArray([segment_color]),
				)
