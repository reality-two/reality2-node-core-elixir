...
class GQL:
	...
	func sentantSend(id = null, event = null, parameters = {}, callback = null, details = "id name", passthrough = {}): _sentantSend(id, event, parameters, callback, details, passthrough)
	
	# Load a swarm of Sentants
	func swarmLoad(definition = null, callback = null, details = "id name", passthrough = {}): _swarmLoad(definition, callback, details, passthrough)
	...
	# --------------------------------------------------------------------------------------------------------------------------------------------------
	# Load a Swarm to the Node from a definition string in YAML, TOML or JSON format.
	# --------------------------------------------------------------------------------------------------------------------------------------------------
	func _swarmLoad(definition, callback, details, passthrough):
		var body = """
		...
		_GQL.mutation(body, _swarmLoad_response, variables, {}, {"callback": callback, "passthrough": passthrough})
	# --------------------------------------------------------------------------------------------------------------------------------------------------
	func _swarmLoad_response(data, passthrough):
		var response = []
		var errors = {}
		if (data.has("errors")):
			errors = data["errors"]
			...
			response = data["data"]["swarmLoad"]
			if (passthrough.callback):
				passthrough.callback.call(response, passthrough.passthrough)
			else:
				print ("SWARM_LOAD: ", response)
	# --------------------------------------------------------------------------------------------------------------------------------------------------
